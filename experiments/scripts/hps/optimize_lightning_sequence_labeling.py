...
from lepiszcze.evaluation_utils import (
    SEQUENCE_LABELING_EVALUATION_MODES_ARGS,
    SequenceLabelingEvaluationMode,
)
...
def run(
    ...
    evaluation_mode: SequenceLabelingEvaluationMode = typer.Option(
        "...", help="Sequence Labeling evaluation mode.", case_sensitive=False
    ),
) -> None:
    ...
    config_space = LightingSequenceLabelingConfigSpace.from_dict(hps_config)
    evaluation_mode_kwargs = SEQUENCE_LABELING_EVALUATION_MODES_ARGS[evaluation_mode.value]
    ...
    optimized_pipeline = OptimizedLightingSequenceLabelingPipeline(
